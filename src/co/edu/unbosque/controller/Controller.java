...
<<<<<<< HEAD
				bos.getMujer().aÒadirMujeres(nombre, usuario, 0, estatura, 0, divorcios, correo);
			} catch (DatosIncompletosException e1) {
				// TODO Auto-generated catch block
				fm.mostrarMensaje("No puede haber un dato en blanco!");
			}catch(NumberFormatException en) {
				fm.mostrarMensaje("Escribe un n√∫mero en estatura!");
			}
			...
			try {
				double estatura = Double.parseDouble(fm.getFrmuj().getTxtEstatura().getText());
				bos.getHombre().aÒadirHombres(nombre, 0, estatura, 0, correo, ingresos, usuario);
=======
				bos.getMujer().a√±adirMujeres(nombre, "", 0, estatura, 0, divorcios, correo);
